	...
	public class DataCreator
	{
		...
		public async Task UpdateData()
		{
			...
			using (var client = await authenticator.GetAuthenticatedClient())
			{
				...
				var data = GenerateData(DateTime.Today.AddYears(-2), DateTime.Today, TimeSpan.FromMinutes(10), 100);
		...
		private async Task CreateData(HttpClient client, Variable variable1, Variable variable2, int? holesEveryXDatapoint = null)
		{
			...
			var data = GenerateData(DateTime.Today.AddYears(-2), DateTime.Today, TimeSpan.FromMinutes(10), 100, holesEveryXDatapoint);
